...
train_image_dir = fullfile(image_dir, 'train');
train_label_dir = fullfile(label_dir, 'train');
validation_image_dir = fullfile(image_dir, 'validation');
validation_label_dir = fullfile(label_dir, 'validation');
test_image_dir = fullfile(image_dir, 'test');
test_label_dir = fullfile(label_dir, 'test');

% Crear subcarpetas si no existen
mkdir(train_image_dir); mkdir(train_label_dir);
mkdir(validation_image_dir); mkdir(validation_label_dir);
mkdir(test_image_dir); mkdir(test_label_dir);

% Función para crear la imagen de la letra "A" en Braille
create_braille_image = @(gray_value) generate_braille_image(image_size, dotRadius, dotSpacing, dotCenters, gray_value, 1);
...
for i = 1:num_train + num_validation + num_test
    ...
    % Crear la imagen base de la letra "A" en Braille (gris claro)
    ...
    img_filename = fullfile(img_dir, sprintf('a_%d.png', image_counter));
    imwrite(transformed_img, img_filename);
    
    % Crear el archivo de etiqueta en formato YOLO
    if ~isempty(label_dir_current)
        label_filename = fullfile(label_dir_current, sprintf('a_%d.txt', image_counter));
        fileID = fopen(label_filename, 'w');
        
        % La clase de la letra "A" es 0 (solo tenemos una clase)
        class_id = 0;
