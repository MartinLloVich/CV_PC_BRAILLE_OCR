...
% Función para crear la imagen de la letra "I" en Braille
create_braille_image = @(gray_value) generate_braille_image(image_size, dotRadius, dotSpacing, dotCenters, gray_value, [2, 4]);
...
for i = 1:num_train + num_validation + num_test
    ...
    % Crear la imagen base de la letra "I" en Braille (gris claro)
    ...
    img_filename = fullfile(img_dir, sprintf('i_%d.png', image_counter));
    imwrite(transformed_img, img_filename);
    
    % Crear el archivo de etiqueta en formato YOLO
    if ~isempty(label_dir_current)
        label_filename = fullfile(label_dir_current, sprintf('i_%d.txt', image_counter));
        fileID = fopen(label_filename, 'w');
        
        % La clase de la letra "I" es 8
        class_id = 8;
