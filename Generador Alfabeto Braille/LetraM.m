...
% Función para crear la imagen de la letra "M" en Braille
create_braille_image = @(gray_value) generate_braille_image(image_size, dotRadius, dotSpacing, dotCenters, gray_value, [1, 3, 4]);
...
for i = 1:num_train + num_validation + num_test
    ...
    % Crear la imagen base de la letra "M" en Braille (gris claro)
    ...
    img_filename = fullfile(img_dir, sprintf('m_%d.png', image_counter));
    imwrite(transformed_img, img_filename);
    
    % Crear el archivo de etiqueta en formato YOLO
    if ~isempty(label_dir_current)
        label_filename = fullfile(label_dir_current, sprintf('m_%d.txt', image_counter));
        fileID = fopen(label_filename, 'w');
        
        % La clase de la letra "M" es 12
        class_id = 12;
